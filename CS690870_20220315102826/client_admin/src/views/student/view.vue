			...
			<el-col v-if="user_group === '管理员' || $check_field('get','teacher_job_number') || $check_field('add','teacher_job_number') || $check_field('set','teacher_job_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="教师工号" prop="teacher_job_number">
						<div v-if="user_group !== '管理员'">
							{{ get_user_session_teacher_job_number(form_sub['teacher_job_number']) }}
							<!--<el-input id="business_name" v-model="form['teacher_job_number']" placeholder="请输入教师工号"-->
							<!--v-if="user_group === '管理员' || (form['student_id'] && $check_field('set','teacher_job_number')) || (!form['student_id'] && $check_field('add','teacher_job_number'))" :disabled="disabledObj['teacher_job_number_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','teacher_job_number')">{{form['teacher_job_number']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['student_id'] && $check_field('set','teacher_job_number')) || (!form['student_id'] && $check_field('add','teacher_job_number'))" id="teacher_job_number" v-model="form_sub['teacher_job_number']" :disabled="disabledObj['teacher_job_number_isDisabled']">
								<el-option v-for="o in list_user_teacher_job_number" :key="o['username']" :label="o['nickname'] + '-' + o['username']"
										   :value="o['user_id']">
								</el-option>
							</el-select>
							<el-select v-else-if="$check_field('get','teacher_job_number')" id="teacher_job_number" v-model="form_sub['teacher_job_number']" :disabled="true">
								<el-option v-for="o in list_user_teacher_job_number" :key="o['username']" :label="o['nickname'] + '-' + o['username']"
										   :value="o['user_id']">
								</el-option>
							</el-select>
						</div>
						<el-select v-else id="teacher_job_number" v-model="form_sub['teacher_job_number']">
							<el-option v-for="o in list_user_teacher_job_number" :key="o['username']" :label="o['nickname'] + '-' + o['username']"
									   :value="o['user_id']">
							</el-option>
						</el-select>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','teacher_name') || $check_field('add','teacher_name') || $check_field('set','teacher_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="教师姓名" prop="teacher_name">
					<el-input id="teacher_name" v-model="form_sub['teacher_name']" placeholder="请输入教师姓名"
							  v-if="user_group === '管理员' || (form_sub['student_id'] && $check_field('set','teacher_name')) || (!form_sub['student_id'] && $check_field('add','teacher_name'))"></el-input>
	...
	export default {
		...
		methods: {
			...
			async get_group_user_teacher_job_number() {
				this.form_sub["teacher_job_number"] = this.user.user_id;
